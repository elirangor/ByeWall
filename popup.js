/* popup.js – ByeWall v1.7 ____________________________________________ */
...
async function loadHistory() {
  ...
  archiveHistory.forEach((item) => {
    ...
    if (isRightToLeft) {
      ...
      formattedDate = `${dt.getDate()}/${dt.getMonth() + 1}/${dt
        .getFullYear()
        .toString()
        .slice(2)}, ${hours}:${minutes}`;
    } else {
      // LTR: time first, then date
      formattedDate = `${hours}:${minutes}, ${dt.getDate()}/${
        dt.getMonth() + 1
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll('input[name="archiveService"]')
    .forEach((radio) =>
